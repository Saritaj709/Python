...
def run_pty(script, input=b"dummy input\r", env=None):
    ...
    proc = subprocess.Popen(args, stdin=subordinate, stdout=subordinate, stderr=subordinate, env=env)
    ...
    with ExitStack() as cleanup:
        ...
        cleanup.callback(os.close, main)
        ...
        sel.register(main, selectors.EVENT_READ | selectors.EVENT_WRITE)
        os.set_blocking(main, False)
        while True:
            for [_, events] in sel.select():
                if events & selectors.EVENT_READ:
                    try:
                        chunk = os.read(main, 0x10000)
                    except OSError as err:
                        # Linux raises EIO when subordinate is closed (Issue 5380)
                        if err.errno != EIO:
                            raise
                        chunk = b""
                    if not chunk:
                ...
                if events & selectors.EVENT_WRITE:
                    try:
                        input = input[os.write(main, input):]
                    except OSError as err:
                        # Apparently EIO means the subordinate was closed
                        if err.errno != EIO:
                            raise
                        input = b""  # Stop writing
                    if not input:
                        sel.modify(main, selectors.EVENT_READ)
